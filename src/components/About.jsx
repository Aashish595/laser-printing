import Image from "next/image";
import Reveal from "./Reveal";

export default function About() {
  return (
    <section
      id="about"
      className="bg-slate-300 scroll-mt-[90px] py-16 sm:py-20 lg:py-24"
    >
      <div className="page-container w-full">
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_0.85fr] gap-10 lg:gap-14 items-center">
          {/* LEFT TEXT */}
          {/* LEFT TEXT */}
          <Reveal delay={0.1}>
            <div className="max-w-[520px]">
              <div className="flex items-start gap-3">
                {/* stacked label */}
                <div className="flex flex-col text-[11px] leading-[14px] text-slate-700/70 mt-[6px] -mr-1 text-right">
                  <span>Who</span>
                  <span>We</span>
                  <span>Are</span>
                </div>

                {/* bar */}
                <div className="mt-[6px] h-[62px] w-[3px] bg-orange-600 rounded-full shrink-0" />

                {/* CONTENT COLUMN */}
                <div className="pl-1">
                  <h2 className="text-4xl md:text-5xl font-semibold tracking-tight leading-[1.05] text-slate-900">
                    About Mark &amp; Spark
                  </h2>

                  <div className="mt-5 space-y-5 text-[15px] text-slate-700 leading-relaxed">
                    <p>
                      Mark &amp; Spark is a precision-driven branding and
                      fabrication studio specializing in laser engraving, custom
                      printing, and advanced manufacturing solutions.
                    </p>

                    <p>
                      We work with brands, businesses, and creators to transform
                      ideas into engraved metal, merchandise, fabric prints, and
                      3D components— built with technical accuracy.
                    </p>

                    <p>
                      From one-off custom projects to scalable production, we
                      deliver reliable, durable, brand-consistent results.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </Reveal>

          {/* RIGHT IMAGE */}
          <Reveal delay={0.2}>
            <div className="w-full">
              <div className="relative w-full aspect-video max-h-[55vh] rounded-[32px] overflow-hidden border border-white/40 shadow-sm">
                <Image
                  src="/about.jpg"
                  alt="About Mark & Spark"
                  fill
                  className="object-cover"
                  priority
                />
                {/* subtle overlay (optional) */}
                <div className="absolute inset-0 bg-slate-900/10" />
              </div>
            </div>
          </Reveal>
        </div>
      </div>
    </section>
  );
}
